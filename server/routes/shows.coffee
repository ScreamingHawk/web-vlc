...
setApiDetails = (show, forceApi=false, callback=null)->
	...
	if show.source?
		# Success
		return callback?()
	# Try MAL first or fail over
	mal.updateApiData show, forceApi, ()->
		if show.api != "omdb" && !show.source?
			# Fail over OMDB
			omdb.updateApiData show, forceApi
		callback?()
...
getFileMeta = exports.getFileMeta = (fPath, fMime, api)->
	# Create meta obj
	fMeta =
		path: fPath
		api: api
	...
	fMeta.mime = fMime
	# Get episode
	episode = fMeta.filename?.match /(?:e|_|-|episode|^)\s*(\d{1,2})/i
	if episode? && !isNaN episode[1]
		fMeta.episode = Number episode[1]
	# Get season from folder
	i = 2
	season = parts[parts.length - i]?.match /(?:s|season)\s*(\d{1,2})$/i
	if season? && !isNaN season[1]
		fMeta.season = Number season[1]
